...
class AcfIconPicker extends acf_field {
	function __construct($settings) {
		...
		$this->path_suffix = apply_filters('acf_icon_path_suffix', '/assets/icons/');
				...
				<div class="acf-icon-picker__img">
					<?php
						if (file_exists($svg)) {
						  ...
						  echo '<img src="' . $svg . '" alt="" loading="lazy"/>';
						  echo '</div>';
						} else{
							echo '<div class="acf-icon-picker__svg">';
	...
	function input_admin_enqueue_scripts() {
		...
		wp_register_script('acf-input-icon-picker', "{$url}/inc/acf/fields/icon-picker/assets/js/input.js", array('acf-input'), $version);
		...
		wp_register_style('acf-input-icon-picker', "{$url}/inc/acf/fields/icon-picker/assets/css/input.css", array('acf-input'), $version);
		wp_enqueue_style('acf-input-icon-picker');
	}
}

new AcfIconPicker($this->settings);

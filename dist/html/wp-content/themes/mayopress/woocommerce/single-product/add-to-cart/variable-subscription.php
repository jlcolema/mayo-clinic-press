...
<form class="variations_form cart" action="<?php echo esc_url( apply_filters( 'woocommerce_add_to_cart_form_action', $product->get_permalink() ) ); ?>" method="post" enctype='multipart/form-data' data-product_id="<?php echo absint( $product->get_id() ); ?>" data-product_variations="<?php echo htmlspecialchars( wcs_json_encode( $available_variations ) ) ?>">
	...
	<?php else : ?>
		<?php if ( user_has_active_subscription_to_product($user_id, $product->get_id()) ) : ?>
        ...
        <?php esc_html_e( 'You have an active subscription to this product already.', 'woocommerce-subscriptions' ); ?>
			...
			<?php echo '<a class="btn btn--primary" href="' . esc_url( wc_get_endpoint_url( 'subscriptions', '', wc_get_page_permalink( 'myaccount' ) ) ) . '">' . __( 'Manage Subscription', 'woocommerce-subscriptions' ) . '</a>'; ?>
		<?php else : ?>
			<p class="country-notification">The Print and Print & Digital versions of the Health Letter are only available in the US and Canada. The Digital version of the Health Letter is available to everyone worldwide. If you are not seeing your region's available versions, please ensure the accurate region is selected in the below country dropdown menu.</p>
			<?php echo do_shortcode('[cbr_select_countries_widget]'); ?>
